...
#include "trt-tensor.cuh"
#include "trt-infer.cuh"
...
void inference(const char *engine_path){
    ...
    string path = engine_path;
    auto engine = TRT::load_infer(path);
    if(engine == nullptr){
    ...
    float* input_data_host   = reinterpret_cast<float*>(input->cpu<float>());
    
    ///////////////////////////////////////////////////
    // letter box
    auto image = cv::imread("workspace/group.jpg");
    // 通过双线性插值对图像进行resize
    ...
    float* output_data_host   = reinterpret_cast<float*>(output->cpu<float>());
...
int main(){
    const char *path = "/home/srb/trtLearning/infer/workspace/yolov5s.onnx";
    const char *engine_path = "/home/srb/trtLearning/infer/workspace/engine.trtmodel";
