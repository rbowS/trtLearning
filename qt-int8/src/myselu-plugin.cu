#include "../include/myselu-plugin.cuh"
#include "NvInfer.h"
...
vector<PluginField> MySELUPluginCreator::mPluginAttributes;
...
// 定义插件类MYSELUPlugin
MySELUPlugin::MySELUPlugin(const string name, const string attr1, float attr3)
: mLayerName(name), mattr1(attr1), mattr3(attr3)
{
    printf("==================== 编译阶段, attr1 = %s, attr3 = %f\n", attr1.c_str(), attr3);
}

MySELUPlugin::MySELUPlugin(const string name, const void *data, size_t length)
:mLayerName(name)
{
...
// 获取该层的输出维度是多少
nvinfer1::DimsExprs MySELUPlugin::getOutputDimensions(
...
// 判断该插件所支持的数据格式和类型
bool MySELUPlugin::supportsFormatCombination(int32_t pos, const PluginTensorDesc* inOut, int32_t nbInputs, int32_t nbOutputs) 
noexcept
{
...
void MySELUPlugin::configurePlugin(
...
IPluginV2* MySELUPluginCreator::createPlugin(const char* name, const PluginFieldCollection* fc) noexcept
{
    ...
    for (int i = 0; i < fc->nbFields; i++)
    {
        if(strcmp(fields[i].name, "attr1") == 0)
        {
            ...
            attr1 = string(cp, cp+fields[i].length);
        }
